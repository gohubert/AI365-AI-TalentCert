...
// ── 本地題庫快取（離線備用 + 多題庫支援）──
let LOCAL_QUESTION_BANKS = {
  'gov-ai': { meta: {}, questions: [] },
  'comptia-secai': { meta: {}, questions: [] }
};

try {
  const govPath = path.join(__dirname, '..', 'data', 'question-bank.json');
  if (fs.existsSync(govPath)) {
    const raw = JSON.parse(fs.readFileSync(govPath, 'utf8'));
    if (raw.exam && Array.isArray(raw.questions)) {
      LOCAL_QUESTION_BANKS['gov-ai'].meta = raw.exam;
      LOCAL_QUESTION_BANKS['gov-ai'].questions = raw.questions.map(normalizeQuestion);
    }
  }

  const secaiPath = path.join(__dirname, '..', 'data', 'comptia-secai-bank.json');
  if (fs.existsSync(secaiPath)) {
    const rawSecai = JSON.parse(fs.readFileSync(secaiPath, 'utf8'));
    if (rawSecai.exam && Array.isArray(rawSecai.questions)) {
      LOCAL_QUESTION_BANKS['comptia-secai'].meta = rawSecai.exam;
      LOCAL_QUESTION_BANKS['comptia-secai'].questions = rawSecai.questions.map(normalizeQuestion);
    }
  }
} catch (err) {
...
/**
 * 從雲端或本地下載練習題（含答案+詳解）
 * @param {string} bankType - 'comptia-secai' 或 'gov-ai'
 * @param {string} mode - 'all' (全題庫順序) 或 'random' (隨機抽題)
 */
async function downloadPracticeExam(bankType = 'comptia-secai', mode = 'all') {
  console.log('[API] Loading practice questions for bank:', bankType, 'mode:', mode);
  return downloadPracticeExamLocal(bankType, mode);
}

/**
 * 本地快取練習題
 */
function downloadPracticeExamLocal(bankType = 'comptia-secai', mode = 'all') {
  const bankKey = bankType === 'gov-ai' ? 'gov-ai' : 'comptia-secai';
  const bankInfo = LOCAL_QUESTION_BANKS[bankKey];

  let source = bankInfo && bankInfo.questions && bankInfo.questions.length > 0 ? bankInfo.questions : [];
  if (source.length === 0) {
    source = LOCAL_QUESTION_BANKS['gov-ai'].questions || [];
  }

  if (source.length === 0) {
    return { success: false, error: '本地無快取題庫' };
  }
  ...
  if (mode === 'random') {
    ...
    // Sequential / Full Bank
    selected = source.map((q, idx) => ({ ...q, id: idx + 1 }));
  }

  const meta = bankInfo.meta || {};
  return {
    success: true,
    data: {
      exam: {
        title: meta.title || (bankKey === 'comptia-secai' ? 'CompTIA SecAI+ Certification Exam' : '公務AI共通核心能力認證'),
        code: meta.code || (bankKey === 'comptia-secai' ? 'CY0-001' : ''),
        level: meta.level || 'Professional',
        subject: meta.subject || (bankKey === 'comptia-secai' ? 'CompTIA SecAI+ 資訊安全與 AI 認證' : 'AI 人工智慧應用'),
        totalTime: 0,
        passingScore: meta.passingScore || (bankKey === 'comptia-secai' ? 750 : 60),
        pointsPerQuestion: meta.pointsPerQuestion || 2,
        bankType: bankKey,
      },
      questions: selected,
      mode: 'practice',
